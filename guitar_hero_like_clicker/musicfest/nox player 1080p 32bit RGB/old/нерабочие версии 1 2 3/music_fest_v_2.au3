...
Func count_pixel_and_otput_to_console_and_do_action($button, $action)
	$current_pixel_state=0
	$pixel_in_memory=0
	$pixel_counter=0
...
Func state_change_reaction($current_pixel_state, $old_pixel_state, $button, $action)
	if ($current_pixel_state = 'empty_empty' or $current_pixel_state = 'clicked_empty') and _
	   ($old_pixel_state = 'empty_tile' or $old_pixel_state = 'clicked_tile' or IsNumber($old_pixel_state)) Then  ; was tile or random became empty
	    If $action Then
		    unclick_action($button)
			normal_click_action($button)
		EndIf
		ConsoleWrite  ("+ " & @ScriptLineNumber & $button & ": UNclick and Normal click" & @LF)
	EndIf
	if ($current_pixel_state = 'empty_blue' or $current_pixel_state = 'clicked_blue') and _
	   ($old_pixel_state = 'empty_tile' or $old_pixel_state = 'clicked_tile' or IsNumber($old_pixel_state)) Then ; was tile or random became blue
	    If $action Then
	...
	EndIf
	if ($current_pixel_state = 'empty_empty' or $current_pixel_state = 'clicked_empty') and _
	   ($old_pixel_state = 'empty_blue' or $old_pixel_state = 'clicked_blue') Then ; was blue became empty
	    If $action Then
			unclick_action($button)
		EndIf
...
count_pixel_and_otput_to_console_and_do_action(3, True)
...
Func hold_click_first()
	Sleep(250)
	ControlSend("NoxPlayer", "centralWidgetWindow", "" , "{down z}")
	; MouseMove(737, 800, 1)
	;MouseDown("left")
EndFunc
...
Func unclick_first()
	Sleep(250)
	ControlSend("NoxPlayer", "centralWidgetWindow", "" , "{up z}")
	; MouseMove(737, 800, 1)
	;MouseUp("left")
EndFunc
...
Func hold_click_second()
	Sleep(250)
	ControlSend("NoxPlayer", "centralWidgetWindow", "" , "{down x}")
	; MouseMove(872, 800, 1)
	;MouseDown("left")
EndFunc
...
Func unclick_second()
	Sleep(250)
	ControlSend("NoxPlayer", "centralWidgetWindow", "" , "{up x}")
	; MouseMove(872, 800, 1)
	;MouseUp("left")
EndFunc
...
Func hold_click_third()
	Sleep(250)
	ControlSend("NoxPlayer", "centralWidgetWindow", "" , "{down c}")
...
Func unclick_third()
	Sleep(250)
	ControlSend("NoxPlayer", "centralWidgetWindow", "" , "{up c}")
	; MouseMove(1007, 800, 1)
	;MouseUp("left")
EndFunc
...
Func hold_click_four()
	Sleep(250)
	ControlSend("NoxPlayer", "centralWidgetWindow", "" , "{down v}")
...
Func unclick_four()
	Sleep(250)
	ControlSend("NoxPlayer", "centralWidgetWindow", "" , "{up v}")
	; MouseMove(1141, 800, 1)
	;MouseUp("left")
EndFunc
